...
namespace ObsidianLauncher.Services
{
    public class ArgumentBuilder
    {
        ...
        private string _authUuid = Guid.NewGuid().ToString("N");
        ...
        private string _launcherVersion = LauncherConfig.VERSION; // Use actual version
        ...
        {
            _logger.Information("Building JVM arguments for version {VersionId} (Instance: {InstancePath})...", mcVersion.Id, instancePath);
            ...
            if (mcVersion.Arguments?.Jvm != null)
            {
                foreach (var argWrapper in mcVersion.Arguments.Jvm)
                {
                    ...
                    {
                        jvmArgs.Add(ReplacePlaceholders(argWrapper.PlainStringValue, mcVersion, classpath, nativesDir, instancePath));
                    }
                    else if (argWrapper.IsConditional)
                    {
                        ...
                        if (AreRulesSatisfied(conditionalArg.Rules, javaRuntime))
                        {
                            ...
                            {
                                jvmArgs.Add(ReplacePlaceholders(conditionalArg.GetSingleValue(), mcVersion, classpath, nativesDir, instancePath));
                            }
                            else if (conditionalArg.IsListValue())
                            {
                                jvmArgs.AddRange(conditionalArg.GetListValue()
                                    .Select(val => ReplacePlaceholders(val, mcVersion, classpath, nativesDir, instancePath)));
                            }
                        }
                    }
                }
            ...
            if (mcVersion.Logging?.Client?.File != null && !string.IsNullOrEmpty(mcVersion.Logging.Client.Argument))
            {
                ...
                string logConfigDir = Path.Combine(_config.AssetsDir, "log_configs");
                ...
                if (File.Exists(logConfigFilePath))
                {
                    ...
                    string loggingArg = ReplacePlaceholders(mcVersion.Logging.Client.Argument, mcVersion, classpath, nativesDir, instancePath)
        ...
        {
            _logger.Information("Building game arguments for version {VersionId} (Instance: {InstancePath})...", mcVersion.Id, instancePath);
                ...
                foreach (var argWrapper in mcVersion.Arguments.Game)
                {
                    ...
                    else if (argWrapper.IsConditional)
                    {
                        ...
                        if (AreRulesSatisfied(conditionalArg.Rules, null))
                        {
                            if (conditionalArg.IsSingleValue())
                            {
                                gameArgs.Add(ReplacePlaceholders(conditionalArg.GetSingleValue(), mcVersion, null, null, instancePath));
                            }
                            else if (conditionalArg.IsListValue())
                            {
            ...
            else if (!string.IsNullOrEmpty(mcVersion.MinecraftArguments))
            {
                ...
                gameArgs.AddRange(legacyArgsRaw.Select(arg => ReplacePlaceholders(arg, mcVersion, null, null, instancePath)));
            }
            else
            {
        ...
        private string ReplacePlaceholders(string argument, MinecraftVersion mcVersion, string classpath, string nativesDir, string instancePath)
        {
            ...
            string gameDirectoryPath = $"\"{Path.GetFullPath(instancePath)}\"";
            string assetsRootPath = $"\"{Path.GetFullPath(_config.AssetsDir)}\""; // Assets are global
            string nativesDirectoryPath = nativesDir != null ? $"\"{Path.GetFullPath(nativesDir)}\"" : "\"${natives_directory}\""; // Natives are per-instance
            string effectiveClasspath = classpath != null ? $"\"{classpath}\"" : "\"${classpath}\"";
        ...
        private bool AreRulesSatisfied(List<ArgumentRuleCondition> rules, JavaRuntimeInfo javaRuntimeForJvmRules)
        {
            ...
            bool anAllowRuleMatchedAndWasSatisfied = false; // Default: if rules are present, an allow rule must match.
            ...
            foreach (var rule in rules)
            {
                ...
                if (conditionMet)
                {
                    ...
                    {
                        _logger.Verbose("Rule matched and DISALLOWED argument due to: OS={RuleOsName}, Features: {Features}",
                            rule.Os?.Name ?? "N/A",
        ...
        private bool CheckOsRule(OperatingSystemInfo osRule, JavaRuntimeInfo javaRuntimeForJvmRules)
        {
            ...
            if (!string.IsNullOrEmpty(osRule.Name))
            {
                ...
                switch (currentOs)
                {
                    ...
                    default: _logger.Warning("CheckOsRule: Unknown current OS type {CurrentOs}", currentOs); return false;
                }
