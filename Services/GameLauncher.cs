...
public class GameLauncher
{
    ...
    /// <returns>
    ///     A Task representing the asynchronous operation. The task completes when the Minecraft process exits. Returns
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var firstElementAddedToBuilder = false;
        ...
        Action<string> appendQuotedArgument = argVal =>
        {
            ...
            if (firstElementAddedToBuilder) argumentsBuilder.Append(' ');
            ...
            var needsQuotes = argVal.Contains(' ') && !(argVal.StartsWith("\"") && argVal.EndsWith("\""));

            if (needsQuotes) argumentsBuilder.Append('"');
        ...
        if (finalArguments.Length < 1024) // Log full if reasonably short
            ...
            _logger.Information("  Full JVM, MainClass & Game Arguments (first 1024 chars): {Arguments}...",
                finalArguments.Substring(0, 1024));
        ...
        try
        {
            ...
            _logger.Information(
                "Minecraft process successfully started with ID: {ProcessId}. Attaching output readers.", process.Id);
            ...
            _logger.Information("Minecraft process (ID: {ProcessId}) has exited with code: {ExitCode}", process.Id,
                process.ExitCode);
        ...
        {
            _logger.Warning(
                "Launch operation was cancelled. Minecraft process (ID: {ProcessId}) may still be running or was terminated.",
                process.Id);
            if (!process.HasExited)
            {
                _logger.Information("Attempting to kill Minecraft process (ID: {ProcessId}) due to cancellation.",
                    process.Id);
                try
                {
                    process.Kill(true); // Kill the process and any child processes it might have spawned
                    _logger.Information("Minecraft process (ID: {ProcessId}) killed successfully due to cancellation.",
                        process.Id);
                }
                catch (InvalidOperationException ioe) when (ioe.Message.Contains("process has already exited"))
                {
                    _logger.Information(
                        "Minecraft process (ID: {ProcessId}) had already exited when kill was attempted after cancellation.",
                        process.Id);
                }
                catch (Exception killEx)
                {
                    _logger.Error(killEx,
                        "Exception while trying to kill Minecraft process (ID: {ProcessId}) after cancellation.",
                        process.Id);
                }
            }
        ...
        {
            _logger.Error(ex,
                "An exception occurred while launching or monitoring the Minecraft process (ID: {ProcessId}).",
                process.Id);
            if (!process.HasExited)
                try
                {
