        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            using var fileStream =
                new FileStream(filePath, FileMode.Create, FileAccess.Write, FileShare.None, 8192,
                    true); // true for async

            var buffer = new byte[8192]; // Standard buffer size
    ...
    private void DeletePartialFile(string filePath, string reasonForDeletion)
    {
        if (File.Exists(filePath))
            try
            {
                ...
                _logger.Warning("Deleted partially downloaded file due to {Reason}: {FilePath}", reasonForDeletion,
                    filePath);
            }
