...
using Serilog;
// For ZipFile and ZipArchive
// Not directly used here, but models might be
...
public class LibraryManager
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.Information("Processing {Count} library entries for version {VersionId}...", mcVersion.Libraries.Count,
            mcVersion.Id);
        ...
        var totalLibraries = mcVersion.Libraries.Count;
        ...
        foreach (var library in mcVersion.Libraries)
        {
            ...
            processedLibraries++;
            ReportLibraryProgress(progress, library.Name, processedLibraries, totalLibraries,
            ...
            if (library.Downloads?.Artifact != null)
            {
                ...
                var artifactLocalPath = Path.Combine(_config.LibrariesDir,
                    artifact.Path.Replace('/', Path.DirectorySeparatorChar));

                ReportLibraryProgress(progress, library.Name, processedLibraries, totalLibraries,
                    $"Ensuring artifact: {Path.GetFileName(artifactLocalPath)}");
                ...
                if (mainArtifactOk)
                {
                    ...
                    _logger.Verbose("Main artifact for {LibraryName} is ready at {Path}", library.Name,
                        artifactLocalPath);
                }
                else
                {
                    _logger.Error("Failed to ensure main artifact for library {LibraryName}. Path: {Path}",
                        library.Name, artifactLocalPath);
            ...
            {
                _logger.Verbose(
                    "Library {LibraryName} has no specified artifact or classifiers in downloads. Assuming it's a conditional/platform-specific parent or already provided.",
                    library.Name);
            ...
            if (mainArtifactOk && library.Natives != null && library.Natives.Any())
            {
                ...
                if (library.Natives.TryGetValue(osName, out var nativeClassifierKey))
                {
                    if (library.Downloads?.Classifiers != null &&
                        library.Downloads.Classifiers.TryGetValue(nativeClassifierKey, out var nativeArtifact))
                    {
                        var nativeJarLocalPath = Path.Combine(_config.LibrariesDir,
                            nativeArtifact.Path.Replace('/', Path.DirectorySeparatorChar));
                        ReportLibraryProgress(progress, library.Name, processedLibraries, totalLibraries,
                            $"Ensuring native: {nativeClassifierKey}");
                        ...
                        {
                            _logger.Information(
                                "Extracting natives for {LibraryName} from {NativeJarPath} to {NativesDir}",
                                library.Name, nativeJarLocalPath, nativesDir);
                            ReportLibraryProgress(progress, library.Name, processedLibraries, totalLibraries,
                                $"Extracting native: {nativeClassifierKey}");
                            nativesOk = ExtractNativeJar(nativeJarLocalPath, nativesDir, library.Extract);
                            if (!nativesOk)
                                _logger.Error(
                                    "Failed to extract natives for library {LibraryName} from {NativeJarPath}",
                                    library.Name, nativeJarLocalPath);
                            else
                        ...
                        {
                            _logger.Error(
                                "Failed to ensure native JAR for library {LibraryName} ({NativeClassifierKey})",
                                library.Name, nativeClassifierKey);
                            nativesOk = false;
                        }
                    }
                    else
                    {
                        _logger.Warning(
                            "Native classifier '{NativeClassifierKey}' specified for OS '{OsName}' in library {LibraryName}, but no corresponding download found in classifiers.",
                ...
                {
                    _logger.Verbose("No specific native classifier for current OS '{OsName}' in library {LibraryName}.",
                        osName, library.Name);
                }
            }

            if (mainArtifactOk &&
                nativesOk) // Only count as successful if both main (if any) and natives (if any) are okay
            {
        ...
        var allSucceeded = successfullyProcessedLibraries == mcVersion.Libraries.Count(IsLibraryApplicable);
        if (allSucceeded)
        {
            _logger.Information(
                "All {SuccessfullyProcessedCount} applicable libraries for version {VersionId} processed successfully.",
                successfullyProcessedLibraries, mcVersion.Id);
        }
        else
        {
            _logger.Error(
                "{FailedCount} out of {ApplicableCount} applicable libraries failed to process for version {VersionId}.",
    ...
    {
        if (library.Rules == null || !library.Rules.Any()) return true; // No rules means allow by default
        ...
        foreach (var rule in library.Rules)
        {
            ...
            if (conditionMet)
            {
                if (rule.Action == RuleAction.Allow)
                    ...
                    (rule.Action ==
                     RuleAction.Disallow) return false; // A disallow rule immediately makes it not applicable
            }
        }
    ...
    private bool CheckOsRule(OperatingSystemInfo osRule)
    {
        ...
        if (!string.IsNullOrEmpty(osRule.Version) && nameMatch) // Only check version if OS name matches
            ...
            _logger.Verbose(
                "OS version rule found ('{OsRuleVersion}') but not implemented for matching against '{CurrentOsVersion}'. Assuming match for now.",
    ...
    private bool ExtractNativeJar(string nativeJarPath, string nativesDir, LibraryExtractRule extractRule)
    {
        try
        {
            ...
            foreach (var entry in archive.Entries)
            {
                ...
                {
                    var excluded = false;
                    foreach (var exclusion in extractRule.Exclude)
                ...
                var destinationPath = Path.Combine(nativesDir, entry.Name); // Use entry.Name to avoid full path issues
                ...
                _logger.Verbose("Extracting native: {EntryFullName} to {DestinationPath}", entry.FullName,
                    destinationPath);
        ...
        CancellationToken cancellationToken, ulong? expectedSize = null)
    {
        ...
        {
            DeletePartialFile(downloadedFilePath, "Download Canceled", fileDescription);
            return false;
        }
        ...
        if (!string.IsNullOrEmpty(expectedSha1))
        {
            ...
            if (cancellationToken.IsCancellationRequested)
            {
                DeletePartialFile(localPath, "Verification Canceled", fileDescription);
                return false;
            }
    ...
    private void DeletePartialFile(string filePath, string reason, string fileDescription)
    {
        if (File.Exists(filePath))
            try
            {
                ...
                _logger.Warning("Deleted file {Description} ({FilePath}) due to: {Reason}", fileDescription, filePath,
                    reason);
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Failed to delete file {Description} ({FilePath}) after error ({Reason})",
                    fileDescription, filePath, reason);
            }
    }
