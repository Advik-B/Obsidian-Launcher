        ...
        try
        {
            ...
            if (args.Length > 0 && !string.IsNullOrWhiteSpace(args[0]))
            {
                ...
                if (args.Length > 2 && !string.IsNullOrWhiteSpace(args[2]))
                {
                    ...
                    Log.Information("Player name for this session from command line: {PlayerName}", cliPlayerNameFromArg);
                }
            }
            Log.Information("Target Minecraft version for setup: {VersionId}", versionIdToLaunch);
            
            // Determine the player name for this session
            string sessionPlayerName = cliPlayerNameFromArg ?? $"Player{Random.Shared.Next(100, 999)}";
            ...
            Log.Information("--- Launching Minecraft {VersionId} for instance '{InstanceName}' (Player: {PlayerName}) ---", 
