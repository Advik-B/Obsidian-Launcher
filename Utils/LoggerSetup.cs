...
namespace ObsidianLauncher.Utils // Correct namespace based on folder structure
{
    public static class LoggerSetup
    {
        ...
        public static void Initialize(
            ObsidianLauncher.LauncherConfig config, // Fully qualified type
            ...
            LogEventLevel fileLevel = LogEventLevel.Verbose) // 'Verbose' in Serilog is similar to 'Trace' in spdlog
        {
            ...
            string logFilePath = Path.Combine(config.LogsDir, "launcher-.log"); // Serilog adds date to filename for rolling
            ...
            try
            {
                ...
                else
                {
                    ...
                    Console.WriteLine($"Warning: LogsDir is not configured in LauncherConfig. File logging will be skipped.");
                }


                Log.Logger = loggerConfiguration.CreateLogger();

                Log.Information("Logger initialized. Console Level: {ConsoleLogLevel}, File Level: {FileLogLevel} (Path: {LogFile})",
                    consoleLevel,
                    fileLevel,
        ...
        public static void SetLevel(string loggerName, LogEventLevel level)
        {
            ...
            Log.Warning("Serilog's SetLevel for a specific context ('{LoggerName}') after initialization is not directly supported in this basic setup. Minimum levels are generally controlled globally or at the sink level. Consider using LoggingLevelSwitch for dynamic control.", loggerName);
