...
// For logging, if needed within this utility
...
public static class OsUtils
{
    ...
    {
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) return OperatingSystemType.Windows;
        if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX)) // macOS
            return OperatingSystemType.MacOS;
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux)) return OperatingSystemType.Linux;
    ...
    public static ArchitectureType GetCurrentArchitecture()
    {
        switch (RuntimeInformation.ProcessArchitecture)
        {
            ...
            default:
                _logger.Warning("Could not determine current architecture. Defaulting to Unknown. Actual: {ActualArch}",
